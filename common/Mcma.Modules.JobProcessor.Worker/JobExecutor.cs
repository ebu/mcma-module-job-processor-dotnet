    ...
    internal class JobExecutor
    {
        ...
        public async Task<Job> StartExecutionAsync(JobReference jobReference, Job job, IJobCheckerTrigger checkerTrigger)
        {
            ...
            try
            {
                ...
                if (!Uri.TryCreate(job.JobProfileId, UriKind.RelativeOrAbsolute, out _))
                    return await FailJobAsync(job, jobExecution, new McmaException($"Provided job profile ID '{job.JobProfileId}' is not valid url."));
                
                // retrieving the jobProfile
                var jobProfile = await ResourceManager.GetAsync<JobProfile>(job.JobProfileId);
                if (jobProfile == null)
                    return await FailJobAsync(job, jobExecution, new McmaException($"Job profile with id {job.JobProfileId} not found."));
                ...
                if (selectedService == null)
                    throw new McmaException("Failed to find service that could execute the " + job.GetType().Name);
