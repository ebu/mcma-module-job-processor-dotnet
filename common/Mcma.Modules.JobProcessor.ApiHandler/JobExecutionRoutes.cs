...
namespace Mcma.Modules.JobProcessor.ApiHandler
{
    ...
    {
        public JobExecutionRoutes(IDataController dataController, IMcmaWorkerInvoker workerInvoker, IOptions<McmaApiOptions> options)
        {
        ...
        private async Task ProcessNotificationAsync(McmaApiRequestContext requestContext)
        {
            ...
            {
                requestContext.SetResponseError((int)HttpStatusCode.BadRequest, $"Unexpected notification from '{notification.Source}'");
                return;
            }
